...
function authoriseDebits(contractAddress, address, delegate) {
    web3.personal.unlockAccount(address, "1234", 60 * 60 * 24, function(err, res) {
        if (err) {
        ...
        token.authorise(delegate, function(err, res){
...
if (process.argv.length > 4) {
  authoriseDebits(process.argv[2], process.argv[3], process.argv[4]);
}
else {
  console.log("Usage: authoriseDebits token address delegate");
}
